...
router.get("/:username", ensureLoggedIn, ensureCorrectUser, async (req, res, next) => {
  try {
    ...
    // Retrieve user's applied job IDs and add them to the response
    const appliedJobs = await Job.getAppliedJobs(user.id);

    // Return user details and applied job IDs
...
router.post("/:username/jobs/:id", ensureLoggedIn, async (req, res, next) => {
  try {
    ...
    // Check authorization and validate input
    const user = await User.applyForJob(username, id);

    // Return success response
    return res.json({ applied: id, jobs: user.appliedJobs });
  } catch (err) {
    return next(err);
  }
});
