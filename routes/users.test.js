...
describe('POST /users/:username/jobs/:id', function () {
	test('works for users: apply for a job', async function () {
	...
	test('not found if user missing', async function () {
		const resp = await request(app).post(`/users/nope/jobs/1`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(404);
	});

	test('not found if job missing', async function () {
		const resp = await request(app).post(`/users/u1/jobs/999`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(404);
	});
});
			...
			users : [
				{
					...
					email     : 'user1@user.com',
					isAdmin   : false,
					jobs      : [ 1 ] // Add the list of applied job IDs
				},
				{
					username  : 'u2',
					firstName : 'U2F',
